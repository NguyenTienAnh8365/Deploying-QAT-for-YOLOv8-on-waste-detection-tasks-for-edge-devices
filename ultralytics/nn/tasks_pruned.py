...
def parse_model_pruned(maskbndict, d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv]:
            ...
            inner_cv1_masks = [maskbndict[inner_cv1_bn_layer_name] for inner_cv1_bn_layer_name in
                               inner_cv1_bn_layer_names]
            inner_cv2_masks = [maskbndict[inner_cv2_bn_layer_name] for inner_cv2_bn_layer_name in
                               inner_cv2_bn_layer_names]
            ...
            cv1_split_sections = [torch.sum(cv1_mask.chunk(2, 0)[0]).int().item(),
