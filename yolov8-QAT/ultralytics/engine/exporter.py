...
class Exporter:
    ...
    def export_engine(self, prefix=colorstr('TensorRT:')):
        ...
        assert self.device.type != 'cpu', 'export running on CPU but TensorRT requires GPU, i.e. use device=0'
        try:
        ...
        check_version(trt.__version__, '>=7.0', hard=True)
        ...
        # Cấu hình tối ưu cho TRT 10
        if self.args.half and builder.platform_has_fast_fp16:
            config.set_flag(trt.BuilderFlag.FP16)
        
        if self.args.int8:
            if builder.platform_has_fast_int8:
                config.set_flag(trt.BuilderFlag.INT8)
                # Với QAT, TRT 10 sẽ tự động dùng các node QDQ trong ONNX
            else:
                LOGGER.info(f'{prefix} device does not support INT8, falling back to FP16/FP32')

        # Thực hiện build
        LOGGER.info(f'{prefix} building engine...')
        serialized_engine = builder.build_serialized_network(network, config)
        
        if serialized_engine is None:
            raise RuntimeError('Could not build the engine. Check TRT logs for layer incompatibility.')
