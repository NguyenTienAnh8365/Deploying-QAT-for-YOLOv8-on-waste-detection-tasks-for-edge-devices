...
class AutoBackend(nn.Module):
    ...
    def __init__(self,
        ...
        if nn_module:  # in-memory PyTorch model
            ...
            dynamic = False
            # Detect TRT API version from version string (attribute access is unreliable
            # in pip-installed TRT builds: both num_io_tensors and num_bindings raise AttributeError)
            trt_major = int(trt.__version__.split('.')[0])
            trt_new_api = trt_major >= 10  # TRT 10.x removed num_bindings, added num_io_tensors

            if trt_new_api:
                # TRT 10.x API: enumerate IO tensors via get_tensor_name loop
                # (avoids num_io_tensors property which is inaccessible in some TRT pip builds)
                all_io_names = []
            ...
            self.__trt_new_api = trt_new_api  # store for use in forward()
        elif coreml:  # CoreML
    ...
    def forward(self, im, augment=False, visualize=False):
        ...
        if self.pt or self.nn_module:  # PyTorch
            ...
            if self.dynamic and im.shape != self.bindings['images'].shape:
                if getattr(self, '_AutoBackend__trt_new_api', False):
                    # TensorRT 10.x
            ...
            if getattr(self, '_AutoBackend__trt_new_api', False):
                # TensorRT 10.x: set tensor addresses then execute
